...
class sms_sts(protocol_packet_handler):
    ...
    def WritePosEx(self, scs_id, position, speed, acc):
        txpacket = [acc, self.scs_lobyte(position), self.scs_hibyte(position), self.scs_lobyte(1000), self.scs_hibyte(1000), self.scs_lobyte(speed), self.scs_hibyte(speed)]
    ...
    def ChangeLevelResponse(self, scs_id, level):
        return self.write1ByteTxRx(scs_id, SMS_STS_RESPONSE_LEVEL, level)
    
    def ProtectCurrent(self, scs_id, protectCurrent):
        return self.write2ByteTxRx(scs_id, SMS_STS_PROTECT_CURRENT, protectCurrent)
    
    def MaxAngleSet(self, scs_id, AngleSet):
        return self.write2ByteTxRx(scs_id, SMS_STS_MAX_ANGLE_LIMIT_L, AngleSet)
    
    def ReadCur(self, scs_id):
    ...
    def SetMaxTorque(self, scs_id, maxtorque):
        txpacket = [self.scs_lobyte(maxtorque), self.scs_hibyte(maxtorque)]
        return self.writeTxRx(scs_id, SMS_STS_TORQUE_LIMIT_L, len(txpacket), txpacket)
